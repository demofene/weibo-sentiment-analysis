...
class TwoStageSentimentTrainer:
    ...
    def stage1_train_binary(self, train_texts=None, train_labels=None, output_dir=None):
    ...
    def build_three_class_model_from_binary(self, binary_model):
        ...
        base_model = getattr(binary_model, 'bert', None) or \
                     ...
                     getattr(binary_model, 'model', None)  
    ...
    def stage2_finetune_three_class(
        ...
        checkpoint_output_dir = self.stage2_checkpoint_dir
        ...
        if base_model_name or not os.path.exists(binary_model_path):
            ...
            if not os.path.exists(binary_model_path):
                print(f"未找到第一阶段模型: {binary_model_path}")
                return None
            binary_model = AutoModelForSequenceClassification.from_pretrained(binary_model_path)
            binary_model = self.build_three_class_model_from_binary(binary_model)
        ...
        trainer, val_dataset = self._create_stage2_trainer(
        ...
        final_output_dir = output_dir if output_dir else self.stage2_dir
        os.makedirs(final_output_dir, exist_ok=True)

        trainer.save_model(final_output_dir)
        self.tokenizer.save_pretrained(final_output_dir)
        self._sanitize_checkpoint_metadata(checkpoint_output_dir)
        self._sanitize_checkpoint_metadata(final_output_dir)
        self._save_evaluation_report(final_output_dir, eval_results, val_labels, pred_labels, report_dict)

        print(f"三分类模型已保存到: {final_output_dir}")
        ...
        print(f"加权 F1: {eval_results['eval_f1_weighted']:.4f}")
        print(f"宏平均精确率: {eval_results['eval_precision_macro']:.4f}")
        print(f"宏平均召回率: {eval_results['eval_recall_macro']:.4f}")
...
if __name__ == "__main__":
    ...
    print("1. Chinese-BERT-wwm (final)")
    print("2. Chinese-RoBERTa-wwm-ext")
    print("3. Chinese-BERT-base")
    ...
    # 指定输出目录（避免覆盖 final）
    if choice == "2":
        output_dir = os.path.join(REPO_ROOT, "weibospider", "models", "roberta-sentiment")
    else:
        output_dir = os.path.join(REPO_ROOT, "weibospider", "models", "trained_model")  # 通用
